...
![图片](D:/LearningNotes.github/设计模式/GameDesignPatterns/Pictures/1.png)
...
+ **原则5：<u>依赖倒置原则:依赖抽象而不要依赖实现</u>**。即：不应让高层组件依赖低层组件，且不管高低，都应依赖抽象。比如上面的PizzaStore的子类就依赖了抽象，在CreatePizza时，参数是抽象的Pizza，而不是一个个具体的类。
  + 如何遵循这个原则？
...
+ 适配器使用被适配的对象的接口，将这些接口用用户使用的接口封装，完成适配。
