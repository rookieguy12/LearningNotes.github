...
+ Input迭代器只能向前移动，一次一步。客户对于迭代器所指物体只能读不能写。例如istream_iterators
+ Output迭代器只能向前移动，一次一步。客户可以写入，而且只能写一次。例如ostream_iterators
+ foward_iterator可以做前面两种迭代器都能做的事，而且可以读写所指物体一次以上。如slist的迭代器
+ Bidirectional_iterator则是在forward的基础上提供了向后移动的操作。如set、multiset、map、multimap的迭代器
+ random_access_iterator。可以在常量时间内移动任意距离。如vector、deque、string的迭代器
+ 不同容器的迭代器类型
