...
+ 思路：这种问题其实就是一个补码的问题，只要保证最后一轮你动手完之后石头数量是4的倍数就行，因此只要你动手前不是4的倍数，你就能拿掉1到3个石头，使得它每轮都变成4的倍数。这是先手的情况，后手的情况就是正好相反，只要是4的倍数，那就必赢。
  ...
  一般地，如果允许拿掉m到n个石头，如果先手，那么石头数量只要不是(m + n)的整数倍就胜利；后手，只要是整数倍就胜利。
...
+ **滑动窗口**：若两个指针指向同一数组，遍历方向相同且不会相交，则也称为**滑动窗口**（两个指针包围的区域即为当前的窗口），经常用于**区间搜索求最值**，比如**快慢指针**。 若两个指针指向同一数组，但是遍历方向相反，则可以用来进行搜索，待搜索的数组往往是排好序的。这常用于解决字符串的子串问题。对于子串、子数组问题，也可以尝试滑动窗口
...
+ 思路二：统计数组前面0的个数，根据当前位置前面有几个0，就可以知道这个位置上的数等会要放在哪个位置上。
...
+ 思路1：使用深度优先搜索。从叶子结点自底向上一个个判断，就能取得最近公共祖先了。因为最近公共祖先的特点只有一个结点能满足，即((leftSon && rightSon) || (root->val == p->val || root->val == q->val) && (lson || rson))，即该结点的左右子树分别有其中一个结点，或者是它本身就等于其中一个结点，同时以它为根的子树里面有另一个结点。而dfs返回值就是表示以root为根的子树里面是否有其中一个结点，即：左右子树里面存在其中一个结点，或者是根本身就等于其中一个结点
  ...
  public:
      ...
      // 返回值表示p，q是否在以root为根的子树内，在查找的过程中顺便更新ans结果。
...
+ 如果使用中序遍历记录结点的值到一个数组上，我们还需要知道p，q相对于current的下标位置，所以可以使用哈希表记录val到下标index的索引，便于查找。
  ...
  public:
      TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
          ...
          inorderTranversal(root, values, myMap);
          
          
          // 这之后的操作就和力扣124的二叉搜索树相差无几了
          TreeNode* current = root;
          int currentIndex = myMap[current->val];
          int lowerPos = min(myMap[p->val], myMap[q->val]);
          int higherPos = myMap[p->val] + myMap[q->val] - lowerPos;			//记录lowIndex和highIndex
          bool tick = currentIndex >= lowerPos && currentIndex <= higherPos;
          while(!tick)
          {
              if (currentIndex < lowerPos) {
                  current = current->right;
              }
              else
                  current = current->left;
              currentIndex = myMap[current->val];
              tick = currentIndex >= lowerPos && currentIndex <= higherPos;
          }
          return current;
      }
...
7. [力扣230]搜索二叉搜索树中第K小的元素。

+ 思路一：[中序遍历]中序遍历正好是有序的遍历，这样就能在遍历的时候得到结果。
...
+ 思路二：[深度优先]统计每个以node结点为根的子树的结点个数。如果，如果numOfLeftNodes ==targetRank - 1；则是当前结点值；如果numOfLeftNodes < targetRank - 1说明目标在右子树中，需要到右子树查找，但是targetRank就不能用了，由于右孩子大于父节点及其左兄弟，所以参考的targetRank应该为targetRank - numOfLeftNodes - 1；
  ...
  public:
      ...
      // 函数的返回值是以root为根的子树的结点个数，任务是在此子树里查找rank为targetRank的结点
      int getNodeNum(TreeNode* root, int targetRank) {
          if (root == nullptr)
              return 0;
          int leftNodeNum = getNodeNum(root->left, targetRank);
          if (leftNodeNum == targetRank - 1)
              res = root->val;
          if (leftNodeNum < targetRank - 1)
              targetRank -= (leftNodeNum + 1);	//如果在右子树，那么应该查找的是右子树里面rank为targetRank - leftNodeNum - 1的结点
          else
              targetRank = -1;					//如果在左子树，说明已经得到了结果，设置targetRank为-1，即可跳过所有上述判断
          return getNodeNum(root->right, targetRank) + leftNodeNum + 1;
      }
  };
  ```
...
+ 思路：对于一条路径，它必然是一个以某结点为根的树，而且这棵树的每个非根节点，至多只有一个子结点。这样一来，我们可以采用递归的做法，如果能求出以每个结点为根的这样的路径，再取得它的最大值即可。显然，在求取root为根的这样的路径时，它可以由root->left，root->right为根的子树得到的单边的路径作为参考。于是，这便可转换为一个递归的问题。
  ...
  public:
      int maxPathSum(TreeNode* root) {
          getSingleRoadMaxSum(root);
          return result;
      }
  
      // 返回以root为根的单路径的最大值
      int getSingleRoadMaxSum(TreeNode* root) {
          if (root == nullptr)
              return 0;
          int leftChildRoadMaxSum = max(getSingleRoadMaxSum(root->left), 0);
          int rightChildRoadMaxSum = max(getSingleRoadMaxSum(root->right), 0);
          // 顺便计算以root为根的子树的最大路径和，用于更新result；
          result = max(leftChildRoadMaxSum + rightChildRoadMaxSum + root->val, result);
          return max(leftChildRoadMaxSum, rightChildRoadMaxSum) + root->val;
      }
  };
  ```

9. [力扣124]二叉搜索树的最近公共祖先。和普通二叉树不同需要根据深度优先的结果不同，二叉搜索树的做法更便捷，只要看value的大小关系就可以确定公共祖先是不是root或者在root的哪个子树里面。

    ```c++
    class Solution {
    public:
        TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
            int lowerValue = min(p->val, q->val);
            int higherValue = p->val + q->val - lowerValue;
            bool tick = root->val >= lowerValue && root->val <= higherValue;
            while(!tick) {
                if (root->val < lowerValue) {
                    root = root->right;
                }
                else
                    root = root->left;
                tick = root->val >= lowerValue && root->val <= higherValue;
            }
            return root;
        }
    };
...
10. [力扣108]将有序数组转换为二叉搜索树。

    给你一个整数数组 nums ，其中元素已经按 升序 排列，请你将其转换为一棵 高度平衡 二叉搜索树。

    高度平衡 二叉树是一棵满足「每个节点的左右两个子树的高度差的绝对值不超过 1 」的二叉树。

+ 思路：简单来说就是利用二分来递归构造，每次利用中位数构造root，再利用左右部分构造左、右子树。
  ...
  public:
      TreeNode* sortedArrayToBST(vector<int>& nums) {
          ...
          root->left = sortedArrayToBST(nums, 0, middle - 1);
          root->right = sortedArrayToBST(nums, middle + 1, size - 1);
          return root;
      }
...
7. 给你一个下标从 0 开始的字符串 text 和另一个下标从 0 开始且长度为 2 的字符串 pattern ，两者都只包含小写英文字母。
...
+ 思路：与其他杂字符无关，只要关注串内的符合pattern的字符。这个插入的字符肯定是放最前面或者最后面，这样得到的结果就是原来的字符的序列个数加上pattern[0]的count和pattern[1]的count这两者中的较大值。

  另外还要考虑pattern[0] == pattern[1]的情况。这种情况的考虑主要是必须把对pattern[1]的判断放在pattern[0]的判断前面。这样在得到第二个pattern[1]的时候才开始计数result
  ...
  public:
      long long maximumSubsequenceCount(string text, string pattern) {
          ...
          for (int i = 0;i < text.length(); ++i) {
              if (text[i] == pattern[1]) {
                  ++secondPatternCount;
                  result += firstPatternCount;
              }
              if (text[i] == pattern[0]) {
                  ++firstPatternCount;
              }
          }
          result += max(firstPatternCount, secondPatternCount);
          return result;
      }
  };
  ```
...
int search(vector<int>& nums, int target) {
    ...
    while(left < right)
    {
        int middle = left + ((right - left)>>1);//切记要打括号，让middle更靠近left
        if (nums[middle] < target)
        {
            left = middle + 1;
        }
        else right = middle;					//由于middle更靠近left，所以存在right = left的可能
    }
...
+ 思路：既然是求连续子数组的和，就可以记录前i个元素的和为priorSum[i]，利用统计priorSum[j] - priorSum[i] = k的个数即可。当然一个更好的办法是将前缀和记录到一个哈希表内，priorSum[j] - k如果在哈希表内，那就说明有这样的一个priorSum[i]，不过得要求哈希表内记录的是前面j-1个元素的各个前缀和，这就要求在计算前缀和的时候动态的修改哈希表的内容，同时利用这个动态的哈希表进行判断。
  ...
  public:
      int subarraySum(vector<int>& nums, int k) {
          ...
          int priorSum = 0;
          int count = 0;
          unordered_map<int, int> priorSumMap;
          priorSumMap[0] = 1;							// 先把0算成1，以将前缀和本身就为k的情况计算在内
          for (int i = 0; i < size; ++i) {
              priorSum += nums[i];
              if (priorSumMap.find(priorSum - k) != priorSumMap.end())
                  count += priorSumMap[priorSum - k];
              ++priorSumMap[priorSum];				// 一边修改前缀和哈希表，一边计算前缀和
          }
          return count;
      }
  };
...
3. [力扣123]\[动态规划]最多可买卖两次，第二次买之前必须卖出第一次的股票

+ 思路：基本思路是基于状态的动态规划。每一天都有若干种状态：

  + 当天啥没干
  + 当天已经买入了第一次股票
  + 当天已经卖出了第一次股票
  + 在卖出了第一次股票的基础上，已经买入了第二次股票
  + 已经卖出了第二次股票

  于是可以通过第i-1天的这些状态来判断第i天的状态下的利润。

  第一种没必要考虑，计buy1为买入了第一次股票的利润，那显然是在第i天，应当是max(buy1[i - 1], -prices[i])，sell1则是当天已经卖出了第一次股票的利润，就是max(sell1[i - 1], buy1[i] + prices[i])；同理，buy2则是max(buy2[i - 1], sell1[i] - prices[i]),sell2就是max(sell2[i - 1], buy2[i] + prices[i])

  最终的结果就是两个sell里动态维护的较大值
  ...
  public:
      int maxProfit(vector<int>& prices) {
          ...
          for (int i = 1; i < n; ++i) {
              ...
              buy2 = max(buy2, sell1 - prices[i]);//不是sell1[i - 1]二是sell1[i]，是允许当天买入卖出
              sell2 = max(sell2, buy2 + prices[i]);
          }
          return sell2;
          //这里直接返回sell2，其实是因为允许当天买入当天卖出，于是sell2的情况实际是包括了sell1的情况。
      }
  };
  ```
...
+ 所以简单来说，就是当fast == slow时，重设置一个节点p从开头和slow一起同步的走，当p == slow时，就是环的入口位置。
+ 时间复杂度O(n)
...
+ 前缀树是一种存储字符串的N叉树，每一个结点代表一个字符串或者字符串前缀，通往不同子结点的路径上有着不同的字符。

+ 一般地，根节点为空，在每个结点上，配置一个哈希表unordered_map<char, TrieNode *>，维护其子结点
...
+ 基本实现
  ...
  class Trie {
  private:
      unordered_map<char, Trie*> childNodes;
      bool isWord;
  public:
      Trie() : isWord(false) {}
      ...
      bool search(string word) {
          ...
          for (char c : word)
          {
              if (current->childNodes.count(c) == 0)
                  return false;
              current = current->childNodes[c];
          }
...
+  01背包：每个东西只有一个
  ...
  for (int i = 0; i < num; ++i) {
      ...
      //从二维来看，是dp[i][j] = max(dp[i - 1][j], dp[i - 1][j - weight[i]] + value[i])，需要上一行的当前列以及上一行左边的数据，为防止从左往右计算时覆盖上一行左侧数据，所以应当从上到下，从右到左
  }
  ```
...
+ 完全背包：每个东西有无数个，可重复选
  ...
  for (int i = 0; i < num; ++i) {
      ...
      //从二维来看，是dp[i][j] = max(dp[i - 1][j], dp[i][j - k*weight[i]] + k*value[i])，它需要的是上一行的数据和当前行的左边的数据，所以应当从上到下，从左到右，得先得到当前行左边的数据，才能计算得到当前位置的数据。对于k的计算，得先得到dp[i][j - (k - 1)*weight[i]]的数据，才能得到dp[i][j - k*weight[i]]的数据，本质上就是得先得到dp[i][j]左侧的数据，这其实在内层循环依次计算的过程中就已经算好了
  }
  ```

### [图算法相关]

1. [Dijkstra，洛谷P1629]有一个邮递员要送东西，邮局在节点1。他总共要送n−1样东西，其目的地分别是节点2到节点n。所有的道路都是单向的，共有m条道路。这个邮递员每次只能带一样东西，并且运送每件物品过后必须返回邮局。求送完这n−1样东西并且最终回到邮局最少需要的时间。

+ 思路：出去是单源最短路径，回来是多对一，但是可以把边的方向取反，从而变成单源最短路径，总之就是两次Dijkstra。
  ...
  void Dijkstra(vector<vector<Edge>>& nodeMap, vector<long long>& distanceList, int start) {
      distanceList[start] = 0;
  
      auto compare = [&](int lhs, int rhs) {
          return distanceList[lhs] > distanceList[rhs];
      };
      priority_queue<int, vector<int>, decltype(compare)> tempQueue(compare);
  
      tempQueue.push(start);
      while(!tempQueue.empty()) {
          int closestNode = tempQueue.top();
          tempQueue.pop();
          for (const Edge& edge : nodeMap[closestNode]) {
              if (distanceList[edge.nextNode] > edge.distance + distanceList[closestNode]) {
                  distanceList[edge.nextNode] = edge.distance + distanceList[closestNode];
                  tempQueue.push(edge.nextNode);
              }
          }
      }
  }
  ...
  int main() {
      int n, m;
      cin >> n >> m;
      vector<vector<Edge>> priNodeMap(n + 1);
      vector<vector<Edge>> reNodeMap(n + 1);
  
      int u, v;
      long long w;
      for (int i = 0; i < m; ++i) {
          cin >> u >> v >> w;
          priNodeMap[u].push_back({v, w});
          reNodeMap[v].push_back({u, w});
      }
  
      int startPoint = 1;
      vector<long long> priDistanceList(n + 1, INT_MAX);
      vector<long long> reDistanceList(n + 1, INT_MAX);
      Dijkstra(priNodeMap, priDistanceList, startPoint);
      Dijkstra(reNodeMap, reDistanceList, startPoint);
  
      long long res = 0;
      for (int i = 2; i <= n; ++i) {
          res += priDistanceList[i] + reDistanceList[i];
      }
